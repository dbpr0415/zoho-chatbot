...
class MemoryService:
    ...
    async def get_context(user_id: str, session_id: str):
        ...
        ctx = (
            f"Current project: {json.dumps(project)}"
            if project
    ...
    async def check_memory_trigger(
        user_msg: str, user_id: str, session_id: str
    ) -> Optional[str]:
    ...
    async def save_long_term(
        user_id: str, session_id: str, user_msg: str, response: str
    ):
        memory = MemoryStore(user_id, session_id)
        msg_lower = user_msg.lower()
        ...
        try:
            ...
            for p in projects:
                ...
                if pname and (
                    pname.lower() in msg_lower or pname.lower() in resp_lower
                ):
        ...
        if "tasks" in resp_lower and any(
            kw in msg_lower for kw in ["list", "show", "tasks", "what"]
        ):
            ...
            if task_names:
                await memory.store_interaction(
                    "last_tasks_viewed", ", ".join(task_names[:5])
                )

        if any(
            kw in resp_lower
            for kw in ["task created", "task updated", "task deleted", "successfully"]
        ):
            await memory.store_interaction(
                "last_action", f"{user_msg[:100]} → {response[:150]}"
            )

        if any(
            w in msg_lower for w in ["always", "prefer", "default", "usually", "i like"]
        ):
