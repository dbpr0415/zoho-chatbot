...
from app.schemas.validation import CreateTaskArgs, DeleteTaskArgs, UpdateTaskArgs
...
async def _resolve_project(
    client: ZohoClient, project_name: str
...
async def _resolve_task(
    client: ZohoClient, project_id: str, task_name: str
) -> Tuple[Optional[str], Optional[str], str]:
...
async def _resolve_assignee(
    client: ZohoClient, project_id: str, assignee_name: str
) -> Tuple[Optional[str], Optional[str], str]:
    members = await client.list_project_members(project_id)
    r_id, name, score, status = resolver.resolve_entity(
        assignee_name, members, name_key="name", id_key="id"
    )
    return r_id, name, status


def _handle_resolution_error(
    entity_type: str, raw_name: str, status: str, matched_name: Optional[str]
) -> str:
...
async def validate_create_task(
    ...
    project_id, real_project_name, p_status = await _resolve_project(
        client, args.project_name
    )
    if not project_id:
        return (
            False,
            _handle_resolution_error(
                "project", args.project_name, p_status, real_project_name
            ),
            {},
        )
    ...
    if args.assignee_name:
        assignee_id, a_name, a_status = await _resolve_assignee(
            client, project_id, args.assignee_name
        )
        if not assignee_id:
            return (
                False,
                _handle_resolution_error(
                    "team member", args.assignee_name, a_status, a_name
                ),
                {},
            )
...
async def validate_update_task(
    user_id: str, raw_args: dict
) -> Tuple[bool, str, Dict[str, Any]]:
    normalized_args = normalizer.normalize_args(raw_args)
    try:
    ...
    task_id, real_task_name, t_status = await _resolve_task(
        client, project_id, args.task_name
    )
    if not task_id:
        return (
            False,
            _handle_resolution_error("task", args.task_name, t_status, real_task_name),
            {},
        )
...
async def validate_delete_task(
    user_id: str, raw_args: dict
) -> Tuple[bool, str, Dict[str, Any]]:
    normalized_args = normalizer.normalize_args(raw_args)
    try:
