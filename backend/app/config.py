...
class ZohoConfig:
    ...
    api_base_url: str = field(
        default_factory=lambda: os.getenv(
            "ZOHO_API_BASE_URL", "https://projectsapi.zoho.in/restapi"
        )
    )
    portal_name: str = field(default_factory=lambda: os.getenv("ZOHO_PORTAL_NAME", ""))
    scopes: str = (
        "ZohoProjects.portals.READ,ZohoProjects.projects.READ,ZohoProjects.tasks.READ,ZohoProjects.tasks.CREATE,ZohoProjects.tasks.UPDATE,ZohoProjects.tasks.DELETE,ZohoProjects.users.READ"
    )
...
class AppConfig:
    ...
    secret_key: str = field(
        default_factory=lambda: os.getenv(
            "APP_SECRET_KEY", "zoho-assistant-secret-key-change-me"
        )
    )
    database_url: str = field(
        default_factory=lambda: os.getenv(
            "DATABASE_URL", "sqlite+aiosqlite:///./zoho_assistant.db"
        )
    )
